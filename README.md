...
| | 脚本模式（默认） | Agent 自主执行 |
| --- | --- | --- |
| 怎么跑 | 跑 `scripts/generate_carbonj_cover.py` | 执行的 Agent 自己读 SOP 端到端完成 |
...
模式偏好记在 `~/.carbonj-cover/config.json`，设一次以后不再问。自主模式完整流程见 [`references/agent-native-flow.md`](references/agent-native-flow.md)。
...
```bash
python3 ~/.claude/skills/carbonj-cover/scripts/generate_carbonj_cover.py \
  --video "<视频路径>" \
  --title "<标题或主题>" \
  --topic "<补充背景>"
```
...
scripts/generate_carbonj_cover.py   脚本模式实现
references/
