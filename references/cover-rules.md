...
这份文件是 `generate_carbonj_cover.py` 默认传给 Gemini 的完整封面设计规范。日常使用 Skill 时不需要手动读取；只有要修改视觉标准、排查提示词遗漏或评审脚本输出时，才打开本文件。

## 核心目标

- 为配置的创作者生成稳定、清楚、干净、精致的小红书和 B 站 AI 工具实操视频封面。
- 默认方向是“真实视频证据 + Apple-like 产品视觉 + 清晰标题”。每张生成图就是最终封面，全程不做人物嵌入或其他本地视觉合成。
- 封面必须突出真实屏幕内容、可读标题、主产品 Logo、内容相关点缀和轻微空间层次。

- 参考截图默认用于提炼证据，不用于整张复刻。最终画面要像“重新设计过的实操证据卡片”，只保留能解释主题的界面信号。
- `openai/gpt-image-2` 必须一次性生成完整无人物最终封面；不能本地贴字、贴 Logo、拼其他图片、裁切改版或视觉修补。
...
- 必须是 exact 2.35:1 horizontal cover；脚本 API 尺寸默认使用 `1920x816`。
- 标题区占左侧约 43%-48%，主屏幕证据占右侧约 52%-57%，都必须在缩略图中保持清晰。
- 标题每行字高约占画面高度 18%-24%，优先两行以内，避免把超宽画布浪费成一行小字。
- 屏幕证据只保留一个大而真实的产品界面或结果画廊，可轻微透视并裁到边缘，不能缩成完整工作区。

### 16:9 YouTube 缩略图

- 必须是 exact 16:9 YouTube thumbnail；脚本 API 尺寸默认使用 `1280x720`。
- 这是独立于视频号/B 站个人空间伴随版的输出，按桌面端视频缩略图优化：主标题和一个清晰可识别的结果或产品界面共同承担第一视觉锚点。
- 标题每行字高约占画面高度 11%-15%，避免长句压成小字；画面主体、Logo 和标题要在缩略图尺寸下快速可辨。

### 9:16 YouTube Shorts 竖版缩略图

- 必须是 exact 9:16 portrait cover；脚本 API 默认尺寸使用 `720x1280`。
- 这是独立于 YouTube `16:9` 横版缩略图的竖版输出，不得从横版裁切或缩放伪造。
- 标题必须先于屏幕证据被看见：标题块放在上方约 `9%-42%`，主屏幕/网页成品放在下方约 `42%-96%`，可轻微重叠形成一套画面秩序。
- 主标题每行横向铺满安全区宽度约 `90%-96%`，每行字高约占画面高度 `8%-12%`；宁可分成两行大字，也不要压成长句小字。
- 主屏幕证据约占画面高度 `55%-70%`，至少一侧被画布边缘裁切；在手机/Shorts 信息流缩小后，产品名、关键结果和主标题仍须可辨。
...
- 质检重点包括：标题可读、标题有内容相关装饰、产品 Logo 像参考图、屏幕物件有裁切感、主证据足够大、画面没有人物或摄像头元素、无完整截图噪音、整体完整。
...
Asset type: <Xiaohongshu exact 3:4 portrait cover / exact 4:3 horizontal primary cover for Xiaohongshu and Bilibili homepage recommendations / Bilibili exact 16:9 personal-space companion cover / WeChat Official Account exact 2.35:1 article cover / YouTube exact 16:9 desktop thumbnail / YouTube Shorts exact 9:16 vertical thumbnail>.
Input images: Image 1 is the selected real screen evidence frame. Image 2 is the product logo if provided.
Primary request: Create one complete person-free Apple-like AI tool tutorial cover. Preserve real product identity and tutorial evidence. Remove people, webcam bubbles, old subtitles, and unrelated clutter.
