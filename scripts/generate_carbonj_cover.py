#!/usr/bin/env python3
"""
Generate carbonj-cover Xiaohongshu and Bilibili covers with Zenmux.

The script reads the carbonj-cover reference rules, asks Gemini to select/plan a
...
def _resolve_skill_dir() -> Path:
    """Locate the installed carbonj-cover skill directory without hardcoding a user.
    ...
    Override with CARBONJ_COVER_SKILL_DIR if needed.
    """
    override = os.environ.get("CARBONJ_COVER_SKILL_DIR", "").strip()
    ...
    candidates += [
        Path.home() / ".codex" / "skills" / "carbonj-cover",
        Path.home() / ".claude" / "skills" / "carbonj-cover",
    ]
...
USER_CONFIG_FILE = Path(
    os.environ.get("CARBONJ_COVER_CONFIG", str(Path.home() / ".carbonj-cover" / "config.json"))
).expanduser()
...
def load_user_config() -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"Invalid carbonj-cover user config: {USER_CONFIG_FILE}: {exc}") from exc
    if not isinstance(payload, dict):
        raise RuntimeError(f"carbonj-cover user config must be a JSON object: {USER_CONFIG_FILE}")
    return payload
...
DEFAULT_API_KEY_FILE = (
    ...
    else Path.home() / ".config" / "carbonj-cover" / "zenmux_api_key"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate carbonj-cover images with Zenmux.")
    ...
    parser.add_argument(
        ...
        help="Optional local file containing the Zenmux API key. Default: user config or ~/.config/carbonj-cover/zenmux_api_key.",
    )
    ...
    parser.add_argument(
        "--wechat-size",
        default="1920x816",
        help="Exact 2.35:1 WeChat Official Account cover size for the image API. Zenmux requires width and height divisible by 16.",
    )
    parser.add_argument(
        "--youtube-size",
        default="1280x720",
        help="Exact 16:9 YouTube thumbnail size for the image API. Zenmux requires width and height divisible by 16.",
    )
    parser.add_argument(
        "--youtube-vertical-size",
        default="720x1280",
        help="Exact 9:16 vertical YouTube Shorts thumbnail size for the image API. Zenmux requires width and height divisible by 16.",
    )
    parser.add_argument(
        "--generation-only",
        action="store_true",
    ...
    parser.add_argument(
        "--aspect",
        choices=("all", "both", "3x4", "4x3", "16x9", "2.35x1", "youtube_16x9", "youtube_9x16"),
        default="all",
        help="Which cover aspect to generate. Default: all six in parallel. Legacy 'both' keeps 3x4 + 4x3.",
    )
    parser.add_argument(
        "--allow-subtitle",
...
def slugify(value: str, fallback: str = "carbonj-cover") -> str:
...
def build_analysis_messages(
    ...
    system_text = (
        ...
        "Use the supplied carbonj-cover skill rules as the design spec. "
        ...
        + "This cover must stay completely person-free: do not add any human, face, portrait, avatar, webcam bubble, mascot, or character. "
        +
        "The final image generator is Zenmux openai/gpt-image-2. The local script extracts frames, "
        "copies files, saves prompts, and calls Zenmux APIs. "
        +
        "It never adds text, pastes product Logos, changes layout, "
        "crops the generated cover, or performs visual repairs locally. "
        +
        ...
        "range and keep the title as the first anchor; for the 2.35:1 WeChat Official Account cover, place the "
        "title in the left 43%-48% and one oversized screen evidence object in the right 52%-57%, with title cap "
        "height about 18%-24% of the canvas height; for the YouTube exact 16:9 thumbnail, make the title and "
        "one recognizable screen result the first two anchors, keep the title cap height about 11%-15%, and "
        "compose for a desktop video thumbnail rather than a phone feed; "
    ...
    user_text = f"""
Task:
Create a complete external Zenmux workflow plan for a carbonj-cover style Xiaohongshu and Bilibili cover set.
...
{{
  ...
  "prompts": {{
    ...
    "2.35x1": {{
      "size": "{args.wechat_size}",
      "prompt": ""
    }},
    "youtube_16x9": {{
      "size": "{args.youtube_size}",
      "prompt": ""
    }},
    "youtube_9x16": {{
      "size": "{args.youtube_vertical_size}",
      "prompt": ""
    }}
  }},
  "quality_checklist": []
...
- The six prompts must explicitly mention exact 3:4, exact 4:3, exact 16:9, exact 2.35:1, exact 16:9 YouTube thumbnail, and exact 9:16 YouTube Shorts thumbnail respectively. The existing 16:9, YouTube 16:9, and YouTube 9:16 prompts are separate platform outputs and must each be present.
...
- Keep the final cover completely person-free: no human face, portrait, avatar, webcam bubble, mascot, character, or person thumbnail.
- Use software UI evidence, product logo, workflow chips, cursor marks, panels, and text hierarchy as the personal-brand signal.
...
- {subtitle_instruction}
- For the 4:3, 16:9, YouTube 16:9, and 2.35:1 horizontal prompts, the main title must be the first visual anchor, while the selected screen evidence remains large and readable. Treat 4:3 as the Bilibili homepage primary upload asset, 16:9 as a separate personal-space companion, YouTube 16:9 as an independent desktop video thumbnail, YouTube 9:16 as a distinct Shorts/mobile thumbnail with the title above a large screen result, and 2.35:1 as the WeChat Official Account article cover: for 2.35:1 use title on the left 43%-48% and one large screen evidence object on the right 52%-57%, both safely away from edges. All prompts must state the title size explicitly (portrait: each line spans ~90%-96% of the safe-area width; landscape: cap height ~11%-15% of canvas height; 2.35:1: cap height ~18%-24% of canvas height) so the title cannot come out small.
"""
...
def post_multipart(
    ...
    boundary = f"----carbonjcover-{uuid.uuid4().hex}"
    body = bytearray()
...
def run_analysis(
    ...
    if args.dry_run:
        analysis = {
            ...
            "selected_frame": {},
            "prompts": {
                "3x4": {"size": args.xiaohongshu_size, "prompt": ""},
                "4x3": {"size": args.landscape_size, "prompt": ""},
                "16x9": {"size": args.bilibili_size, "prompt": ""},
                "2.35x1": {"size": args.wechat_size, "prompt": ""},
                "youtube_16x9": {"size": args.youtube_size, "prompt": ""},
                "youtube_9x16": {"size": args.youtube_vertical_size, "prompt": ""},
            },
...
def requested_aspects(args: argparse.Namespace) -> tuple[str, ...]:
    if args.aspect == "all":
        return ("3x4", "4x3", "16x9", "2.35x1", "youtube_16x9", "youtube_9x16")
    if args.aspect == "both":
        return ("3x4", "4x3")
    return (args.aspect,)
...
def aspect_size(args: argparse.Namespace, aspect_key: str) -> str:
    return {
        "3x4": args.xiaohongshu_size,
        "4x3": args.landscape_size,
        "16x9": args.bilibili_size,
        "2.35x1": args.wechat_size,
        "youtube_16x9": args.youtube_size,
        "youtube_9x16": args.youtube_vertical_size,
    }[aspect_key]
...
def prompt_sidecar_text(
    ...
    return f"""# CarbonJ Cover Prompt Sidecar

Use: Xiaohongshu and Bilibili carbonj-cover external Zenmux workflow
Aspect: {aspect_key}
Size: {size}
...
def main() -> None:
    ...
    if not args.title.strip():
        fail("A confirmed title is required before generating a cover. Pass it with --title.")
    ...
    work_dir = base_dir / f"{stem}.carbonj-cover"
